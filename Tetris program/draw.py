...
def draw_next_shape(shape, win):
    ...
    for i, line in enumerate(format):
        ...
        for j, column in enumerate(row):
            if column == '0':
                pygame.draw.rect(win, shape.get_color(), (sx + j*block_size, sy + i*block_size, block_size, block_size), 0)

    win.blit(label, (sx + 10, sy - 30))


def draw_window(win, grid, score=0, lines_eliminated = 0, gameLevel=1, play_mode="Player", game_mode="Normal"):
